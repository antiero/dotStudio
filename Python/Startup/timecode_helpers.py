...
class PasteShotInfoDialog(QtGui.QDialog):
  ...
  def setupDialogUI(self):
    ...
    self._clipboardTCInfoLabel = QtGui.QLabel("Select which timecode to Paste:")
    ...
    if len(self._destinationShots) == 1:
      self._buttonbox.button(QtGui.QDialogButtonBox.Ok).setText("Paste")

    if len(self._destinationShots) > 1:
      numDestShots = len(self._destinationShots)
      self._buttonbox.button(QtGui.QDialogButtonBox.Ok).setText("Paste to %i shots" % numDestShots)
    
    self._buttonbox.button(QtGui.QDialogButtonBox.Ok).setToolTip(str(self._destinationShots))
