...
class CustomSpreadsheetColumns(QtCore.QObject):
  ...
  gCustomColumnList = [
    ...
    { 'name' : '#Versions', 'cellType' : 'readonly'},
    { 'name' : 'ActiveVersion', 'cellType' : 'readonly'},
    { 'name' : 'Localised', 'cellType' : 'readonly'},
    { 'name' : 'Multi-view', 'cellType' : 'readonly'},
  ...
  def getData(self, row, column, item):
    ...
    if currentColumn['name'] == '#Versions':
      return str(item.numVersions())

    if currentColumn['name'] == 'ActiveVersion':
      return str(item.currentVersion())

    if currentColumn['name'] == 'Localised':
      return str(item.source().isLocalised())

    if currentColumn['name'] == 'Multi-view':
      return str(item.source().hasMultipleViews())
...
# This is a convenience method for returning QtWidgets.QActions with a triggered method based on the title string
